        ...
        {jobs.map((job) => (
          ...
          >
            <div className="space-y-3">
              ...
              <p className="text-text-muted text-sm pt-2 line-clamp-2 max-w-2xl leading-relaxed">{job.description}</p>
            </div>
            <button 
              onClick={() => setSelectedJob(job)}
              className="flex items-center gap-2 px-6 py-3 bg-gray-100 hover:bg-primary hover:text-white rounded-full font-medium transition-all text-sm shrink-0 cursor-pointer"
            >
              Apply Now
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-6 z-50 animate-fade-in">
          <div className="glass-card glass-border rounded-3xl p-8 max-w-lg w-full relative max-h-[90vh] overflow-y-auto bg-white/90">
            <button 
              ...
              className="absolute top-6 right-6 p-2 text-text-muted hover:text-text-main transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
              ...
              <>
                ...
                <form onSubmit={handleApply} className="space-y-4">
                  ...
                  <button 
                    type="submit" 
                    disabled={applying}
                    className="w-full py-3.5 bg-gradient-to-r from-primary to-accent text-white rounded-xl font-medium shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 group disabled:opacity-50 mt-6 cursor-pointer"
                  >
